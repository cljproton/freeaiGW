...
/** 反滥用（无第三方）：蜜罐 + 提交时序。返回错误文案；通过返回 null */
const MIN_ELAPSED_MS = 2500;
const MAX_ELAPSED_MS = 600_000;
function botcheckError(body: Record<string, unknown>, now: number): string | null {
  // 蜜罐：真实用户不可见，自动填充的机器人会填写
  if (typeof body.website === "string" && body.website.length > 0) return "检测到可疑提交，请刷新后重试";
  // 提交时序：过快或过旧视为机器人
  const gotTs = Number(body.got_ts ?? "");
  const elapsed = now - gotTs;
  if (!Number.isFinite(gotTs) || elapsed < MIN_ELAPSED_MS || elapsed > MAX_ELAPSED_MS) return "安全校验失败，请刷新后重试";
  return null;
}
...
ROUTER.get("/", async (c) => {
  ...
  return c.html(<AuthPage pow={pow} gotTs={Date.now()} mode={mode} />);
});
...
ROUTER.post("/register", async (c) => {
  ...
  if (!r1.ok) return c.html(<AuthPage pow={pow} gotTs={gotTs} mode="register" error={r1.message} />);
  const r2 = validatePassword(password);
  if (!r2.ok) return c.html(<AuthPage pow={pow} gotTs={gotTs} mode="register" error={r2.message} />);

  const ipCount = await countIpRegsToday(c.env, ip);
  if (ipCount >= (Number(c.env.MAX_REGISTER_PER_IP_PER_DAY) || 3)) {
    return c.html(<AuthPage pow={pow} gotTs={gotTs} mode="register" error="该 IP 今日注册次数过多，请明日再试" />);
  }

  const botErr = botcheckError(body as Record<string, unknown>, Date.now());
  if (botErr) return c.html(<AuthPage pow={pow} gotTs={gotTs} mode="register" error={botErr} />);
  const okPow = await powVerify(c.env, String(body.pow_id ?? ""), String(body.pow_salt ?? ""), String(body.pow_n ?? ""));
  if (!okPow) return c.html(<AuthPage pow={pow} gotTs={gotTs} mode="register" error="安全校验失败，请刷新后重试" />);

  if (!USERNAME_RE.test(username)) {
    return c.html(
      <AuthPage pow={pow} gotTs={gotTs} mode="register" error="用户名需为 3-20 位字母、数字或下划线" />,
    );
  }

  const exists = await getUserByUsername(c.env, username);
  if (exists) {
    return c.html(<AuthPage pow={pow} gotTs={gotTs} mode="register" error="用户名已存在" />);
  }
...
ROUTER.post("/login", async (c) => {
  ...
  if (await loginBlocked(c.env, ip)) {
    return c.html(<AuthPage pow={pow} gotTs={gotTs} mode="login" error="失败次数过多，请 10 分钟后再试" />);
  }

  const botErr = botcheckError(body as Record<string, unknown>, Date.now());
  if (botErr) return c.html(<AuthPage pow={pow} gotTs={gotTs} mode="login" error={botErr} />);
  const okPow = await powVerify(c.env, String(body.pow_id ?? ""), String(body.pow_salt ?? ""), String(body.pow_n ?? ""));
  if (!okPow) return c.html(<AuthPage pow={pow} gotTs={gotTs} mode="login" error="安全校验失败，请刷新后重试" />);
  ...
  if (!user || !(await pbkdf2Verify(password, user.password_hash))) {
    ...
    return c.html(<AuthPage pow={pow} gotTs={gotTs} mode="login" error="用户名或密码错误" />);
  }
